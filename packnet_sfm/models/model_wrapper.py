...
class ModelWrapper(torch.nn.Module):
    ...
    def training_step_consist(self, batch, *args):
        """Processes a training batch."""
        batch = stack_batch(batch)
        output = self.model.forward_con(batch, progress=self.progress)
        return {
            'loss': output['loss'],
            'metrics': output['metrics']
        }
...
def setup_model(config, prepared, **kwargs):
    ...
    if model.network_requirements['depth_net']:
        ...
        model.add_depth_net2(setup_depth_net(config.depth_net, prepared))
        model.add_depth_net3(setup_depth_net(config.depth_net, prepared))
