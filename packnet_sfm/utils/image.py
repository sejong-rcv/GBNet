...
def flip_model(model, image, flip):
    ...
    if False:
        if flip:
            dd= [flip_lr(inv_depth.unsqueeze(0)) for inv_depth in model(flip_lr(image))]
            return [torch.cat((dd[0],dd[1]),dim=0)]
        else:
            return [model(image)]
    else:
        
        if flip:
            # dd= [flip_lr(inv_depth.unsqueeze(0).unsqueeze(0)) for inv_depth in model(flip_lr(image))]
            # return [torch.cat((dd[0],dd[1]),dim=0)]

            output= model(flip_lr(image))
            return [flip_lr(inv_depth) for inv_depth in output]
        else:
           
            output=model(image)
            return [inv_depth for inv_depth in output]#[i.unsqueeze(1) for i in output]
########################################################################################################################
