...
########################################################################################################################
    ...
    def reduce_photometric_loss(self, photometric_losses):
        ...
        return photometric_loss
    def reduce_consistency_loss(self, consistency_losses):
        """
        Combine the photometric loss from all context images
        ...
        def reduce_function(losses):
            if self.photometric_reduce_op == 'mean':
                ...
                raise NotImplementedError(
                    'Unknown photometric_reduce_op: {}'.format(self.photometric_reduce_op))
        # Reduce photometric loss
        consistency_loss = sum([reduce_function(consistency_losses[i])
                                for i in range(self.n)]) / self.n
        # Store and return reduced photometric loss
...
########################################################################################################################
    ...
    def forward(self, depthnet,image, context, inv_depths,
        ...
        consistency_losses = [[] for _ in range(self.n)]
        ...
        for j, (ref_image, pose) in enumerate(zip(context, poses)):
            ...
            with torch.no_grad():
                pre_depth=depthnet(ref_warped[0])
            consistency_loss = self.calc_photometric_loss(pre_depth, inv_depths)
            ...
            for i in range(self.n):
                ...
                consistency_losses[i].append(consistency_loss[i])
            # If using automask
            if self.automask_loss:
        ...
        loss += self.reduce_consistency_loss(consistency_losses)
